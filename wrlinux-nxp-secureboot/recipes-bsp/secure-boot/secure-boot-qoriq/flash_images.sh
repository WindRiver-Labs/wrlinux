...
#
# Supported platforms:
# LS1021ATWR, LS1012ARDB, LS1012AFRWY, LS1043ARDB, LS1046ARDB, LS1088ARDB, LS2088ARDB, LX2160ARDB
#
...
if test $board = ls1021atwr; then
	...
	mcfw_img=$image_path/mc_10.12.0_ls1088a.itb
    fi
    ...
    if test -z "$mcfw_img"; then
	mcfw_img=$image_path/mc_10.12.0_ls2088a.itb
    fi
    if test -z "$mcdpc_img"; then
    ...
    if test -z "$mcfw_img"; then
	mcfw_img=$image_path/mc_10.12.0_lx2160a.itb
    fi
    if test -z "$mcdpc_img"; then
...
elif test $board = ls1012ardb -o $board = ls1046ardb -o $board = ls1088ardb  -o $board = lx2160ardb; then
...
if test $board = ls1046ardb -o $board = ls1088ardb -o $board = lx2160ardb; then    # probe QSPI flash
    if test $bank = other; then
...
if test $board = ls1043ardb -o $board = ls1046ardb -o $board = ls1088ardb -o $board = ls2088ardb; then
    if load $bd_type $bd_part $img_load_addr $phy_img && $pt_off_cmd $addr_eth $efs && $erase_cmd $addr_eth +$filesize && $write_cmd $img_load_addr $addr_eth $filesize; then
...
# DPAA2 MC firmware
if test $board = ls1088ardb -o $board = ls2088ardb -o $board = lx2160ardb; then
    if load $bd_type $bd_part $img_load_addr $mcfw_img && $pt_off_cmd $addr_mcfw $efs && $erase_cmd $addr_mcfw +$filesize && $write_cmd $img_load_addr $addr_mcfw $filesize; then
	echo Success: flashed $mcfw_img to $addr_mcfw
    else
	echo Failed to flash $mcfw_img
...
elif test $board = ls1088ardb -o $board = ls2088ardb -o $board = lx2160ardb; then
