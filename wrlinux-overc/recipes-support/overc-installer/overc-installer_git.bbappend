...
python overc_installer_rewrite_uri() {
    ...
    if os.path.exists("%s/git/overc-installer.git" % (d.expand(layer_path))):
        src_uri = src_uri.replace("git://github.com/OverC/overc-installer.git;branch=master", "git://${LAYER_PATH_wrlinux-overc}/git/overc-installer.git;branch=wr-10.20-20191211;protocol=file")
