...
SRC_URI = "${SOURCEFORGE_MIRROR}/openhpi/openhpi-${PV}.tar.gz \
           ...
           file://add-libnetsnmp-when-link.patch \
           file://run-ptest \
...
inherit autotools pkgconfig ptest systemd

FILES_${PN} = "${bindir}/* ${sbindir}/* ${libexecdir} ${libexecdir}/* \
               ${libdir}/lib*.so.* ${libdir}/${BPN}/lib*.so.* ${base_libdir}/lib*.so.* \
               ${sysconfdir} ${sharedstatedir} ${localstatedir} \
               ${base_bindir}/* ${base_sbindir}/* ${datadir}/${BPN}"  

FILES_${PN}-dev += "${includedir} ${libdir}/${BPN}/lib*.la"      
...
FILES_${PN}-staticdev += "${libdir}/lib*.a ${libdir}/${BPN}/lib*.a"
...
pkg_prerm_${PN} () {
   ${sysconfdir}/init.d/openhpi stop
}
